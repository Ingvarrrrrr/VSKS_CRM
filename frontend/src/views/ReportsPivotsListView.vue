<template>
  <v-container fluid class="pa-4">
    <div class="d-flex align-center mb-4">
      <v-icon size="28" class="mr-2">mdi-pivot</v-icon>
      <h2 class="text-h5">Сводные</h2>
      <v-spacer />
      <v-btn color="primary" prepend-icon="mdi-plus" to="/reports/pivots/new">Новая сводная</v-btn>
    </div>
    ...
    <v-card>
      ...
      <div v-else-if="!loading" class="text-center text-grey pa-8">
        Нет сохранённых сводных. Нажмите «Новая сводная» чтобы создать.
      </div>
    </v-card>
    ...
    <v-dialog v-model="deleteDialog" max-width="400">
      <v-card>
        <v-card-title>Удалить сводную?</v-card-title>
        <v-card-text>«{{ deleteTarget?.name }}» будет удалена без возможности восстановления.</v-card-text>
        <v-card-actions>
          <v-spacer />
...
onMounted(async () => {
  loading.value = true
  try {
    const result = await apiFetch<any>('/api/report-configs/?kind=pivot')
