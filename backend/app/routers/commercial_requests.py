...
async def list_commercial_requests(
    ...
    q = select(CommercialRequest).options(selectinload(CommercialRequest.recipients)).order_by(CommercialRequest.id.desc())
...
async def update_commercial_request_recipient_status(
    recipient_id: int,
    ...
    recipient = (
        await db.execute(select(CommercialRequestRecipient).where(CommercialRequestRecipient.id == recipient_id))
    ).scalar_one_or_none()
    if not recipient:
