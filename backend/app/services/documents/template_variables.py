...
TEMPLATE_VARIABLES = [
    ...
    ("", "  Спец-формы позиций (item-forms-accommodation-transport.md, шаг 4): "
        "«Проживание»/«Перевозки автобусом»/«Питание» — форма договора закупки.", "", ""),
    ("{{item.form_summary}}", "Человекочитаемое описание позиции спец-формы (пусто для обычной позиции)",
        "{{item.form_summary}}", "Стандартный четырёхместный, 8 номеров × 6 600,00 ₽ × 3 сут."),
    ("{{item.form_summary}}", "То же, пример для «Питание», режим 'simple'", "{{item.form_summary}}",
        "10 чел. × 3 приёма/день × 5 дн. × 250,00 ₽"),
    ("{{item.form_summary}}", "То же, пример для «Питание», режим 'menu'", "{{item.form_summary}}",
        "10 чел. × 2 дн., 6 приёмов — 1 700,00 ₽ на человека, итого 17 000,00 ₽"),
    ("{{item.extra.room_category}}", "Проживание: категория номера", "{{item.extra.room_category}}", "Стандартный четырёхместный"),
    ("{{item.extra.price_basis}}", "Проживание: цена за 'room' или 'person'", "{{item.extra.price_basis}}", "room"),
    ("{{item.extra.rooms}}", "Проживание: количество номеров", "{{item.extra.rooms}}", "8"),
    ("{{item.extra.persons}}", "Проживание/перевозки/питание: количество человек", "{{item.extra.persons}}", "32"),
    ("{{item.extra.nights}}", "Проживание: количество суток", "{{item.extra.nights}}", "3"),
    ("{{item.extra.place_from}}", "Перевозки: пункт отправления", "{{item.extra.place_from}}", "Москва"),
    ("{{item.extra.place_to}}", "Перевозки: пункт назначения", "{{item.extra.place_to}}", "Курск"),
    ("{{item.extra.depart_at}}", "Перевозки: дата и время подачи (ISO)", "{{item.extra.depart_at}}", "2026-05-07T08:00:00"),
    ("{{item.extra.work_hours}}", "Перевозки: часы в работе", "{{item.extra.work_hours}}", "5"),
    ("{{item.extra.supply_hours}}", "Перевозки: часы подачи (по умолчанию 2)", "{{item.extra.supply_hours}}", "2"),
    ("{{item.extra.hourly_rate}}", "Перевозки: ставка за час", "{{item.extra.hourly_rate}}", "1500"),
    ("{{item.extra.cost_mode}}", "Перевозки: расчёт 'hours' или 'trip'", "{{item.extra.cost_mode}}", "hours"),
    ("{{item.extra.trip_cost}}", "Перевозки: стоимость рейса вручную", "{{item.extra.trip_cost}}", "12000"),
    ("{{item.extra.meals_per_day}}", "Питание, режим 'simple': приёмов пищи в день (по умолчанию 3)", "{{item.extra.meals_per_day}}", "3"),
    ("{{item.extra.days}}", "Питание: количество дней", "{{item.extra.days}}", "5"),
    ("{{item.extra.mode}}", "Питание: режим ввода 'simple' или 'menu'", "{{item.extra.mode}}", "menu"),
    ("{{item.extra.menu}}", "Питание, режим 'menu': [{day, meals:[{name, description, price}]}] — цена за приём НА ЧЕЛОВЕКА (сырая структура, для печати использовать item.menu_lines)", "", ""),
    ("", "  Полная раскладка меню питания по дням (food-menu-editor.md) — пусто для всех форм, кроме food+mode=menu.", "", ""),
    ("{%p for l in item.menu_lines %}{{ l }}{%p endfor %}", "Абзацы меню по дням (одна строка на день)", "{%p for l in item.menu_lines %}{{ l }}{%p endfor %}",
