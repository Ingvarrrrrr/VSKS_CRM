...
status: Импорт ФЭО: алерты «позиция стала подразделом» для ручного разбора (строки 31/216 боевого файла) + прежний алерт чужого родителя (211/214) — приёмка на файле владельца пройдена, не запушено. Дальше: шаблоны загрузки закупок проживания/перевозок от владельца, волна 4c доп. соглашений, синк графа
last_updated: "2026-09-09T16:45:00Z"
progress:
  total_phases: 22
  completed_phases: 12
  total_plans: 81
...
Фаза шла руками мимо GSD (одноразовые `patch_templates.py` / `fix_tr_tags.py` в корне,
испорченный текст в 8 шаблонах). Сделана полная пересборка из образцов — решение пользователя.

- Отслеживаемый идемпотентный сборщик `backend/templates/build/` (sources/docxedit/rules_common/rules/build).
  Замена **по смещениям внутри `<w:t>`** после нормализации tracked changes вместо регулярки поверх
  склеенных runs — корневая причина старой порчи. Повторная сборка байт-идентична (sha256).
- Ветвления R1–R10: НДС 20/5/без, третьи лица, ЮЛ/ИП, доставка/самовывоз, этапность ГПХ,
  ретроактивность ст. 425, соглашение о субсидии, срок действия, email, сроки-рекомендации.
- Статика ВСКС (ИНН/КПП/ОГРН, «Козеев», номера соглашений, грантодатель) вычищена во всех 8.
- `{%tr for %}` в сметах рамочного ремонта; зашитая «НДС 18%» → `{{ vat_rate }}`.
- Миграция `t7u8v9w0x1y2` (голова единственная, inspector-guard): 12 колонок `purchases`
  + `contractors.personal_account`. 8 из них жили в модели и в БД, но не были ни в одной миграции.
- Приёмка: `backend/tests/test_contract_templates.py` — **48 passed** (8 шаблонов × 6 комбинаций).

⚠️ **Методика:** метрики «сколько бланков / сколько подстановок» дали ложный PASS трижды —
они зелёные при испорченном тексте. Рендерить только уникальными сентинелами (иначе корректная
подстановка неотличима от статики), сентинелы без `<`, `>`, `&` — docxtpl не экранирует.

Закоммичено и запушено (2026-07-29): `d1ddc8d` база ОКПД2/Фабриканта — её миграции
`r5s6t7u8v9w0` и `s6t7u8v9w0x1` лежали неотслеживаемыми, а голова Phase 28 от них зависит;
`764f612` сама Phase 28; `1bb2c47` запись в TASKS.md + удаление мёртвого
`make_contract_template.py`. Одноразовые скрипты удалены. Легаси `contract_fadm.docx` и
`contract_template_source.docx` мёртвые и сломанные, в рантайме не используются —
из области видимости теста исключены явным списком doc_type (глобить нельзя).

## 2026-08-02 — Разбор «почему 8 форм договоров», план слияния 8 → 5 УТВЕРЖДЁН, код НЕ писан

Пользователь ожидал один договор с галочками и спросил, зачем восемь. Разбор показал, что
унификация уже сделана, но по другой оси: **тип договора = файл** (услуги / поставка / ГПХ /
ремонт — разный предмет, разные главы ГК), **условия сделки = галочки внутри файла**
(НДС, ЮЛ/ИП, третьи лица, ретро, этапность, доставка — правила R1–R6).

Замер сходства (Jaccard по множествам абзацев с нормализацией нумерации):

| Пара | Совпадение | Вывод |
|---|---|---|
| `services_small` / `services_food` | 96.2% | сливаемо |
| `services_small` / `services_large` — **тело** | **93.3%** | сливаемо |
| те же — блок «Методические рекомендации» | 12.0% | это и есть «отчётность» |
| `gph_individual` / `gph_individual_rid` | 92.3% | сливаемо |
| `repair_vehicle` / `repair_framework` | 54.4% | раздельно |
| `goods_single` / услуги | 1.7% | раздельно |

Пользователь был прав: «малая/большая отчётность» — один и тот же договор с разной методичкой,
приложенной к ТЗ (18 абзацев против 247 с Приложениями №1–№6 **к самой методичке**). Тело
ссылается только на «Приложение №1», по 15 раз в обеих формах, на №2–№6 не ссылается вообще →
блок методички подменяется целиком, ручная нумерация тела не сдвигается.

⚠️ **Методика:** первый прогон сабагента был построен на `difflib.quick_ratio` — эта метрика
сравнивает мультимножества символов и на однотипном юридическом русском даёт 90%+ ВСЕГДА
(93% для договора поставки против договора услуг, у которых общего 1.7%). Все её выводы о
сливаемости оказались ложными. Контроль: гнать метрику на заведомо несвязанной паре.

Проверено, что уже реализовано и строить заново не нужно: своя форма договора на субсидию
(`PUT/GET/DELETE /{subsidy_id}/templates/{doc_type}`, приоритет «шаблон субсидии → глобальный»)
и копирование всех шаблонов из соседней субсидии (`POST /{sid}/templates/copy-from/{source_sid}`,
`subsidy_approvers.py:243`, кнопка `SubsidiesView.vue:2605`).

План: `C:\Users\1\.claude\plans\cozy-weaving-flask.md` — утверждён, **реализация не начата**
(прервано сразу после утверждения). Голова alembic на момент планирования — `w1x2y3z4a5b6`.
