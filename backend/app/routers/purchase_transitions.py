...
async def transition_status(
    ...
    # затем (2026-08-10) расширен на КАЖДЫЙ forward-переход.
    #
    # План п.3б (2026-08-11), владелец: «привязанные закупки сравниваются с
    # ПЛАНОМ… а вот уже план сравнивается с ФЭО. Это две разные истории» +
    # «Запрет остаётся там, где владелец его и просил — на движении закупки
    # дальше: заключение договора и последующие стадии блокируются» — гейт
    # СУЖЕН обратно: превышение плана над ФЭО не должно мешать чисто плановым
    # переходам (wishes → plan_schedule, plan_schedule → work_in_progress —
    # закупка ещё не «объявлена» контрагенту, договор не подписан). Держим
    # его РОВНО там, где просил владелец — начиная с перехода в «Договор»
    # (contracted) и на всех последующих стадиях (ordered/delivered/paid),
    # т.е. target_idx >= CONTRACTED_IDX. assert_no_unapproved_excess
    # по-прежнему проверяет ОБА вида превышения (план>ФЭО и факт>план).
    # OWNER_ROLES (superadmin/account_owner) уже обошли ВСЮ функцию раньше
    # (см. bypass в начале) — этот гейт видят все остальные роли, включая
    # ADMIN_ROLES (в отличие от PUT /api/purchases/{pid}, где admin_override —
    # explicit query-флаг только для ADMIN_ROLES; здесь такого параметра нет,
    # обход только через OWNER_ROLES).
    CONTRACTED_IDX = STATUS_ORDER.index("contracted")
    if target_idx > current_idx and target_idx >= CONTRACTED_IDX:
