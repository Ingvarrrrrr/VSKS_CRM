...
- [x] 17-04-PLAN.md - migrate 78 require_role call-sites to require_tab/require_action across 21 routers (D-01b/06)
...
- [x] 17-09-PLAN.md - router guards meta.tab_key + E2E regression + phase sign-off checkpoint (D-01a/05/09)
...
### Phase 28: Шаблоны договоров по субсидиям ✅ COMPLETED (2026-07-29)

**Directory:** `.planning/phases/28-contract-templates-per-subsidy/`

**Context:** `BACKEND-AUDIT.md`, `INVENTORY.md`, `USER-COMMENTS.md` (68 комментариев юриста из образцов = фактическое ТЗ), `28-10-REBUILD-PLAN.md` (управляющий документ)

**Goal:** 8 форм договоров генерируются из данных закупки без ручной доводки, одинаково корректно для всех трёх министерств (ФАДМ / Минпрос / Минтруд).

**Предыстория:** работа шла руками мимо GSD — в корне репозитория лежали одноразовые `build_templates_28.py`, `patch_templates.py`, `fix_tr_tags.py` и др. `patch_templates.py` склеивал runs регуляркой и подставлял значения не в те слоты (`«Исполнитель», в лице info@vsks.ru100000___`), реквизиты ВСКС и «Козеев» были зашиты статикой во все 8 шаблонов, ветки НДС 5% не существовало вовсе.

**Locked decisions (пользователь, 2026-07-28):**
1. Подход — **полная пересборка из образцов**, текущие .docx выбрасываются (не точечный ремонт).
2. Субсидии — **один комплект шаблонов с ветвлениями**, per-subsidy загрузка остаётся только для исключений. Подтверждено диффом: ФАДМ/Минпрос/Минтруд структурно не расходятся, отличия сводятся к переменным.

**Success Criteria:**
1. Отслеживаемый идемпотентный сборщик `backend/templates/build/` собирает все 8 форм из образцов; повторный запуск даёт байт-идентичный результат ✅
2. Замены идут по смещениям внутри `<w:t>` после нормализации tracked changes — корневая причина старой порчи устранена ✅
3. Статика ВСКС (ИНН/КПП/ОГРН, «Козеев», номера соглашений, наименование грантодателя) вычищена во всех 8 ✅
4. Ветвления R1 (НДС 20/5/без), R2 (третьи лица), R3 (ЮЛ/ИП), R4 (доставка/самовывоз), R5 (этапность ГПХ), R6 (ретроактивность ст. 425), R7–R10 ✅
5. Табличные циклы `{%tr for %}` в сметах рамочного договора на ремонт; устаревшая ставка «НДС 18%» заменена на `{{ vat_rate }}` ✅
6. Приёмка **по содержимому**: `backend/tests/test_contract_templates.py` — 48 кейсов (8 шаблонов × 6 комбинаций флагов), рендер сентинелами + поиск порчи текста ✅
7. Миграция `t7u8v9w0x1y2` на 13 колонок Phase 28 (идемпотентная, единственная голова) ✅

**Методика приёмки — важно:** проверки «сколько бланков / сколько подстановок» не годятся, они зелёные при испорченном тексте и дали ложный PASS трижды. Рендер обязателен с уникальными маркерами-сентинелами вместо значений (иначе корректная подстановка неотличима от зашитой статики), сентинелы без `<`, `>`, `&` — docxtpl не экранирует.

**Задачи:** T1–T9 — см. `28-10-REBUILD-PLAN.md`

---

### Phase 29: Vehicle Fleet — учёт автотранспорта (Имущество → Автотранспорт)

**Directory:** `.planning/phases/29-vehicle-fleet/` (TBD)

**Goal:** Полноценный модуль учёта автотранспорта ВСКС и партнёрских организаций: реестр ТС (51+ машина из xlsx), карточка машины с фото/документами/ремонтами, ежедневный пробег и журнал заправок с авторасчётом расхода топлива, генерация путевых листов (легковой ф.3 / грузовой ф.4-С), водительские права в карточке сотрудника, дашборд по орг / расходу / пробегу / стоимости.

**Контекст (из brief 2026-05-19):**
- Реестр от Голичкова: `Доработки/реестр_транспорта_от_Голичкова_обновление_042026.xlsx` (51 ТС, 24 поля).
- Марки: Митсубиши Делика, КАМАЗ, УАЗ Патриот, TATA, ВАЗ, лодки/моторы, квадроциклы, прицепы.
- Регионы эксплуатации: Ростов, ЦУ, Иркутск, Луганск, Тульская, Мариуполь, Москва.
- F4 из 04_TODO.md «Имущество → Автотранспорт» — поглощается этой фазой.

**Locked decisions (через AskUserQuestion 2026-05-19):**
- D-01 Workflow: полный GSD-цикл (discuss → plan → execute), не fast/quick.
- D-02 Навигация: новая вкладка AppBar «Имущество» (mdi-warehouse) → tabs «Автотранспорт» / «Оборудование» (заглушка) / «Прочее» (заглушка).
- D-03 Глубина учёта топлива: полный учёт (ежедневный одометр + норма расхода + журнал заправок + стоимость + путевые листы).
- D-04 Водители: `User.can_drive` boolean + поля ВУ; видны из всех org.

**Scope:**
1. Модель: `Vehicle`, `VehicleAttachment` (фото/документы), `VehicleRepair` (+ её attachments), `VehicleOdometer` (ежедневный пробег), `FuelLog` (заправки), `Trip` (путевой лист), `VehicleFieldHistory` (облачка-комментарии).
2. User extension: `can_drive` + license_series/number/categories/issued_at/expires_at + medical_cert_expires_at.
3. Импорт 51 строки из xlsx-реестра.
4. CRUD UI: `/property/vehicles` (список) + `/property/vehicles/:id` (карточка с табами Общее/Документы/Фото/Ремонты/Пробег/Заправки/Путёвки).
5. Шаблоны .docx путевых листов через docxtpl (легковой / грузовой / специальный).
6. Дашборд `/property/vehicles/dashboard` с виджетами (машины по орг, расход топлива/пробег/часы за период, деньги на бензин/ремонт).
7. Уведомления о просрочке (ОСАГО / медсправка / ВУ) — баннеры + опц. Tasks.
8. Permissions: новый tab `vehicles` в permission_tabs + actions.

**Зависит от:** —
**Unblocks:** будущие подмодули «Имущество / Оборудование» и «Имущество / Прочее».

**Success Criteria:**
1. 51 ТС из xlsx виден в `/property/vehicles` с фильтрами по орг/типу/состоянию.
2. В карточке машины можно: загрузить фото (несколько), приложить СТС/ПТС/прочие документы, открыть/скачать каждый, посмотреть историю изменений каждого поля.
3. Добавить ремонт с заказ-нарядом + фото + стоимостью; стоимость суммируется в карточке.
4. Ввести одометр на дату → авторасчёт литров и стоимости по норме + цене последней заправки; журнал заправок ведётся отдельно.
5. Сгенерировать путевой лист (.docx) для конкретной машины с конкретным водителем; форма выбирается по `vehicle.type` (легковой/грузовой/специальный).
6. Сотрудник с `can_drive=true` + заполненными правами появляется в селекте водителей путёвки; иначе скрыт.
7. Дашборд показывает: машин по орг (bar), расход топлива (line по дням/неделям/месяцам/годам), деньги на бензин и ремонт (KPI + bar по машинам), пробег за период.
8. Просрочка ОСАГО / медсправки / ВУ → баннер в карточке + опц. Task «Продлить ОСАГО {гос.номер}».
...
**Scope (6 пунктов):**
1. **Drag & Drop для закрывающих документов** — перетаскивание файлов прямо в блок закрывающих документов карточки закупки (сейчас только диалог выбора файла).
2. **Подсветка изменений (diff-tracking)** — в карточке задачи/закупки визуально выделять поля, изменённые с последнего просмотра/другим пользователем.
3. **Автосохранение + Undo/Redo (Ctrl+Z/Y)** — стек отмены поверх существующего autosave в карточке закупки; откат отдельного изменения без потери остальных.
4. **Синхронизация договор ↔ закупка** — изменения реквизитов договора (номер/дата/сумма) отражаются в связанной закупке и наоборот, без ручного дублирования.
...
**Success Criteria:**
1. Файл можно перетащить мышью в блок закрывающих документов — загружается как через диалог.
2. Изменённые поля в карточке визуально помечены; отметка сбрасывается после просмотра.
3. Ctrl+Z/Ctrl+Y откатывают/возвращают правки полей в карточке закупки до сохранения на сервер.
4. Правка contract_number/contract_date/contract_price с любой стороны видна с обеих сторон без ручного повтора.
