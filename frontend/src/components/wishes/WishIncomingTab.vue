      ...
      <div class="d-flex align-center" style="gap:4px" @click.stop>
        ...
        <template v-if="item.status === 'submitted'">
          ...
          <template v-else>
            ...
            <v-btn size="x-small" variant="tonal" color="primary" @click.stop="$emit('open-edit', item)">
              Согласовать
            </v-btn>
          </template>
        </template>
...
// WishIncomingTab.vue — вкладка «На согласование мне». Дословный перенос шаблона
