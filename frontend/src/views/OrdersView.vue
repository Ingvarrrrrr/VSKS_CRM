<template>
  <v-container fluid class="pa-6">
    ...
    <!-- Владелец (2026-09-17): открыт фильтр статуса, который обычные фильтры
         ...
         ?status=wishes) — плашка объясняет, почему эти строки обычно не
         видны, чтобы не выглядело так, будто реестр их "прячет" без причины.
         'split' сюда больше не ведёт (владелец, решение 21.09,
         corrections-21-09.md П1): родительские записи разделённых закупок
         скрыты из реестра совсем, см. backend/app/routers/purchases.py. -->
...
// Владелец (2026-09-17): "заявки не в работе" скрыты в реестре обычными
// фильтрами — эта плашка появляется, только когда открыт именно такой
// служебный фильтр (например, по ссылке из диалога удаления субсидии), и
// объясняет причину прямым текстом без жаргона статусов.
// 'split' сюда больше не ведёт (владелец, решение 21.09, П1): родительские
// записи разделённых закупок скрыты СОВСЕМ, ?status=split реестр
// игнорирует и всегда отдаёт пусто — показывать для него баннер незачем.
