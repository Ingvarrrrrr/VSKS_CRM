...
async def test_plan_vs_items_mismatch_warns_when_amount_from_feo_fallback(db_session):
    """Строка-ЗАГОЛОВОК (без «Плановой позиции») без плановых кол-ва/цены —
    план строки посчитан старым фолбэком из чисел «по ФЭО»
    (feo_qty × feo_price). У той же категории в этом же импорте есть ОТДЕЛЬНАЯ
    строка с настоящей позицией Ур.5 и другой суммой → warning
    ...
    (1).xlsx»): раньше этот же эффект получался и когда feo_qty/feo_price
    стояли в строке С «Плановой позицией» — с тех пор такие числа принадлежат
    самой позиции, а не категории (см.
    test_feo_import_row_numbers_belong_to_item.py), поэтому строка-источник
    фолбэка здесь целенаправленно БЕЗ item_name."""
    subsidy = await _make_subsidy(db_session)
    try:
        rows = [
            mk_row(
                lvl2="Направление I8", lvl3="Категория I8",
                feo_qty="2", feo_price="400000",  # план категории = 800 000, фолбэк
            ),
            mk_row(
                lvl2="Направление I8", lvl3="Категория I8",
                item_name="Позиция I8", plan_sum="100",  # своя сумма Ур.5 = 100 → расхождение
            ),
        ]
        result = await _import(db_session, subsidy.id, rows)
        assert result["errors"] == []
