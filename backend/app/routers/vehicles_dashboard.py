...
РАЗРЕЗАНО (Правило №5, сессия 2026-09-08): роутер был 1728 строк, ядро осталось
здесь (transfer-history-recent, kpi, fuel-canister, in-repair,
maintenance-warning, by-org, fuel-by-period, state-donut, top-expenses).
Роутеры-соседи с тем же префиксом /api/vehicles-dashboard:
  vehicles_dashboard_drill.py   — /drill, /expiring-docs-drill
  vehicles_dashboard_summary.py — /all-vehicles-summary, /by-region,
                                   /driver-reports, /filter-counts
  vehicles_dashboard_fines.py   — /fine-leaders, /fines-summary, /fines-by-filial
Общие хелперы видимости/дедупликации/state-словарей вынесены в
app/services/fleet_dashboard_helpers.py — единый источник для всех четырёх
модулей (Правило №6), не дублировать и не заводить вторые копии формул.
"""
