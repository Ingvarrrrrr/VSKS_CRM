<template>
  <v-container fluid class="pa-4">
    <div class="d-flex align-center mb-4">
      <v-icon size="28" class="mr-2">mdi-table</v-icon>
      <h2 class="text-h5">Реестры</h2>
      <v-spacer />
      <v-btn color="primary" prepend-icon="mdi-plus" to="/reports/lists/new">Новый реестр</v-btn>
    </div>
    ...
    <v-card>
      ...
      <div v-else-if="!loading" class="text-center text-grey pa-8">
        Нет сохранённых реестров. Нажмите «Новый реестр» чтобы создать.
      </div>
    </v-card>
    ...
    <v-dialog v-model="deleteDialog" max-width="400">
      <v-card>
        <v-card-title>Удалить реестр?</v-card-title>
