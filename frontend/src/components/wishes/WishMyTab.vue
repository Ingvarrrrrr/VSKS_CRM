        ...
        <template #item.actions="{ item }">
          <div class="d-flex align-center" style="gap:4px" @click.stop>
            ...
            <template v-else-if="item.status === 'approved'">
              <v-btn
                v-if="ctx.isManagerOrAdmin.value"
                size="x-small"
                variant="tonal"
                color="primary"
            ...
            <v-card variant="outlined" class="h-100 d-flex flex-column" hover @click="$emit('open-edit', w)">
              ...
              <v-card-actions class="py-1" @click.stop>
                <v-menu>
                  ...
                  <v-list density="compact">
                    <v-list-item prepend-icon="mdi-image" title="С фото" @click="$emit('download-excel', w, true)" />
                    <v-list-item prepend-icon="mdi-image-off" title="Без фото" @click="$emit('download-excel', w, false)" />
                  </v-list>
                </v-menu>
                ...
                <v-menu v-else-if="w.status === 'converted' && w.purchase_id && (w.purchases?.length || 0) > 1">
                  ...
                  <v-list density="compact">
                    <v-list-item v-for="p in w.purchases" :key="p.id" @click.stop="ctx.goToPurchase(p.id)">
                      <v-list-item-title>
                        {{ purchaseMenuLabel(p) }}
                        <v-chip v-if="p.stopped_at" size="x-small" color="error" variant="tonal" class="ml-1">остановлена</v-chip>
                      </v-list-item-title>
                ...
                <v-btn
                  ...
                  color="purple"
                  prepend-icon="mdi-cart-arrow-right"
                  @click.stop="ctx.goToWishPurchases(w)"
                >
                  {{ wishPurchasesLabel(w) }}
                </v-btn>
              </v-card-actions>
...
// WishMyTab.vue — вкладка «Мои заявки» (таблица + карточки). Дословный перенос
// шаблона (203-581) из WishesView.vue (FAB «Создать» вынесен в WishesView.vue как
// сосед, чтобы корень этого компонента остался единственным элементом — родитель
// читает DOM через $el для RegistryExportButton::get-capture-el без defineExpose).
