                  ...
                  <template v-for="node in visibleFeoNodes" :key="node.id">
                    ...
                    >
                      ...
                      <td class="feo-td feo-td-num">
                        ...
                        <!-- «Заметный сигнал превышения» (план zany-fluttering-mountain.md, 2026-08-11):
                             владелец — «в случае превышения должна отображаться данная закупка и показать,
                             что из-за неё всё превысило». Раньше плашка выше называла ТОЛЬКО сумму — виновник
                             нигде не был виден. Крупно, адресно, с прямой ссылкой на закупку. -->
...
interface FeoDisplayPrefs {
  ...
  // Пункт 4 плана zany-fluttering-mountain.md: строка плана раскрыта по умолчанию,
  // если под ней есть закупка (см. applyDefaultPlannedExpansion) — но явное решение
  // пользователя СВЕРНУТЬ строку обязано пережить перезагрузку и не быть перезаписано
  // дефолтом. Раз "развёрнуто" теперь может появиться и БЕЗ клика пользователя, самого
  // expandedPlannedItems недостаточно, чтобы отличить «дефолт» от «пользователь свернул»
  // (оба случая — id отсутствует в массиве). collapsedPlannedItems — id, которые
...
// п.«Заметный сигнал превышения», 2026-08-11) — см. backend/app/services/feo_plan.py
// ::find_excess_culprit за методом определения; null, пока превышения нет либо оно
// согласовано (сервер считает виновника только для узлов с неснятым excess_amount).
...
// Пункт 4 плана zany-fluttering-mountain.md (2026-08-11): владелец решил, что закупки под
// плановой строкой «нет вовсе», если её не видно без клика — раскрыто ПО УМОЛЧАНИЮ, когда
// под строкой есть хотя бы одна закупка (см. applyDefaultPlannedExpansion). Явное решение
// пользователя свернуть строку (collapsedPlannedItems, см. интерфейс FeoDisplayPrefs) имеет
// приоритет над этим правилом — именно поэтому toggle ниже пишет в ОБА множества.
...
// «Заметный сигнал превышения» (план zany-fluttering-mountain.md, 2026-08-11) — виновная
// закупка, из-за которой узел вышел за финансирование ФЭО. Приходит готовой с бэкенда
// (см. app.services.feo_plan.find_excess_culprit, GET /api/feo-categories/plan-tree) — сервер
// заполняет её ТОЛЬКО пока excess_amount не согласован, поэтому дополнительно проверять
// excessApprovalFor тут не нужно: approved-случай уже отдаёт culprit=null сам по себе.
...
/* «Заметный сигнал превышения» (план zany-fluttering-mountain.md, 2026-08-11) — сознательно
   КРУПНЕЕ и заметнее соседних .feo-plan-note (10px, тонкий текст): владелец жаловался, что
   превышение «теряется среди чисел» — эта плашка обязана читаться с первого взгляда, поэтому
   контрастный красный фон/рамка, а не просто цвет текста. */
.feo-excess-culprit {
