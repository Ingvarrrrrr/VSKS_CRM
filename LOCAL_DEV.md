...
**Правки самого `autodeploy.sh` вступают в силу СО СЛЕДУЮЩЕЙ выкладки.**
Скрипт первым делом делает `git pull`, который подменяет в том числе и его
самого, — но текущий запуск доигрывает прежней версией. Поэтому изменение
порядка шагов деплоя всегда требует двух заходов: первый привозит новый
скрипт, второй по нему работает. Проверять применение надо не по коммиту,
а по строкам в `/var/log/vsks-deploy.log`: если формулировки в логе прежние —
работал старый скрипт.

Тот же класс ловушки стоил простоя 2026-09-04: смонтированный в контейнер
одиночный файл `nginx.conf` остался прежним после `git pull`, потому что
монтирование файла держится за конкретный файл, а не за путь. Общее правило:
**и скрипт деплоя, и смонтированные файлы обновляются не тогда, когда
обновился репозиторий.** Проверять по тому, что реально видит запущенный
процесс, а не по содержимому репозитория.
