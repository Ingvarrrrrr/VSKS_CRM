...
async def match_planned_items(
    ...
    # Кап поднят с 10 до 25 (владелец, 2026-09-16, жалоба «поиск по плановым — только
    # поле для ввода, без ранжированного списка»): интерактивный поиск по ВСЕЙ субсидии
    # (FeoPlannedItemsSelect.vue::useFeoPlannedSearch) вызывает этот же эндпоинт БЕЗ
    # feo_category_id — кандидатов в большой субсидии больше, чем в старом варианте
    # (подсказки внутри уже выбранной категории), урезание до 10 отбрасывало бы часть
    # совпадений до того, как фронт успеет сгруппировать точные/частичные/остальные.
    # Батч-подбор (bulk-match, тот же эндпоинт) продолжает запрашивать limit=5 — там
    # нужен только лучший кандидат на позицию, отдельного ограничения не требует.
    top_k = max(1, min(body.limit, 25))
