...
export interface FeoMatchCandidate {
   ...
   *  ветки предков). С 2026-09-02 /feo-planned-items/map снова требует совпадения
   *  категорий (или потомка) — привязка к такому кандидату отклоняется обычному
   *  пользователю (409 с объяснением) и разрешена только суперадмину (с
   *  уведомлением согласовавших/ответственного), категория позиции закупки при
   *  этом больше НЕ переносится молча. same_category используется и для группировки
   *  «своя категория / другая категория» в UI, и как сигнал, что выбор такого
   *  кандидата обычным пользователем будет отклонён сервером. */
