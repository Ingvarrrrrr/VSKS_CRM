...
Осталось: боевые `backend/templates/contract_*.docx` перезаписаны, но **ничего не закоммичено
и не запушено**; корневые одноразовые скрипты и `.tmp_p28*` пока на месте (удаление
untracked-файлов необратимо — ждёт подтверждения). Легаси `contract_fadm.docx` и
