...
class WishOut(BaseModel):
    ...
    # Владелец (2026-09-02): ТЗ позиции дороже её КОНКРЕТНОЙ плановой позиции
    # (FeoPlannedItem) — тоже больше не блокирует согласование заявки, а
    # регистрируется как запрос на согласование превышения (см.
    # app.services.tz_excess_approval.register_tz_excess_approvals). Формат
    # элементов — тот же, что у GET /api/plan-excess (см. plan_excess._approval_dict).
    # Пустой список — превышений над плановыми позициями нет.
    tz_excess_approvals: List[dict] = []
