...
def _require_vat_rate_for_doc(p, doc_type: str) -> None:
    ...
    Владелец (закупка РЕЕ-2026-00918, 2026-09-16): «проставил всем позициям
    5% вручную, но лист согласования требует основание "не облагается"» —
    режим Purchase.vat_mode == 'per_item' («НДС для каждой позиции») ставит
    ставку построчно в PurchaseItem.vat_rate, а эта проверка читала ТОЛЬКО
    шапку (vat_applicable/vat_rate/vat_exemption_article), которая в этом
    режиме не заполняется вовсе — документ отказывал с тем же кодом, что и
    при реально пустом НДС. Ветка на vat_mode ниже — та же самая проверка
    «система ничего не придумывает сама», просто источник данных другой.
    """
    if doc_type not in VAT_RATE_PRINTED_DOC_TYPES:
        return
    if (getattr(p, "vat_mode", None) or "uniform") == "per_item":
        _require_per_item_vat_rate_for_doc(p, doc_type)
        return
    if p.vat_applicable:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "hint": (
                    ...
                    "Откройте панель «Позиции закупки» и укажите ставку НДС: "
                    "0, 5, 7, 10, 20 или 22 % (5 и 7 % — для УСН с 2025 года) — "
                    "как указано у поставщика. Если НДС на самом деле не "
                    "облагается — выберите «Не облагается» и укажите статью НК РФ; "
                    "для самозанятых исполнителей и договоров ГПХ с физлицом "
                    "статья подставляется сама."
                ),
...
def _require_per_item_vat_rate_for_doc(p, doc_type: str) -> None:
    """Ветка _require_vat_rate_for_doc для Purchase.vat_mode == 'per_item'
    («НДС для каждой позиции») — ставка НДС не в шапке закупки, а построчно
    в PurchaseItem.vat_rate (см. Phase 26-U-3).

    Правило то же самое, что и у шапочной проверки («система ничего не
    придумывает про НДС сама»), просто на уровне позиции: у ВСЕХ позиций
    закупки должна быть заполнена своя ставка (в т.ч. «Не облагается» — см.
    комментарий у ItemsTableFlat.vue/ItemsCardsView.vue/ItemsTableStages.vue
    про VAT_RATE_OPTIONS с value=null для «Не облагается» — в этом режиме
    пустое поле трактуется ОДИНАКОВО что для «ещё не выбрано», что для «явно
    не облагается»; отдельного флага «выбрано, но без НДС» построчная модель
    ...
    if not missing:
        return
    n = len(missing)
    word = "позиции" if n == 1 else "позиций"
    raise HTTPException(
        422,
        detail={
            "code": "VAT_RATE_REQUIRED",
            "message": f"У {n} {word} не указана ставка НДС",
            "hint": (
                f"Режим «НДС для каждой позиции» — у {n} {word} в панели "
                "«Позиции закупки» не выбрана ставка. Укажите ставку 0, 5, 7, "
                "10, 20 или 22 % в строке позиции (5 и 7 % — для УСН с 2025 "
                "года), либо отметьте «Не облагается»; для самозанятых "
                "исполнителей и договоров ГПХ с физлицом основание "
